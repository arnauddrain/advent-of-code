...
function pathFindingMaxDistance(memory, position, alreadyVisited) {
  ...
  for (var direction of [
    ...
    if (!alreadyVisited[hash(newPosition)]) {
      ...
      if (output !== 0) {
        const test = pathFindingMaxDistance(memory, newPosition, alreadyVisited);
        if (test >= maxPath) {
          maxPath = test + 1;
        }
...
function pathFindingDistance(memory, position, alreadyVisited) {
  ...
  for (var direction of [
    ...
    if (!alreadyVisited[hash(newPosition)]) {
      const output = run(memory, direction.code);
      if (output === 2) {
        return pathFindingMaxDistance(memory, newPosition, {});
      }
      if (output === 1) {
